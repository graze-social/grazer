...
def test_all_sources_present():
    records = [
        make_record(
            ...
            embed={
                "images": [{"alt": "Alt text"}],
                "external": {"title": "Title", "description": "Desc"},
            },
        )
    ]
...
def test_split_into_k_lists_k_larger_than_elements():
    with pytest.raises(
        ValueError, match="k cannot be greater than the number of elements"
    ):
...
def test_get_all_links_normal():
    records = [
        {
            "commit": {
                "record": {"embed": {"external": {"uri": "http://example.com/link1"}}}
            }
        },
        {
            "commit": {
                "record": {"embed": {"external": {"uri": "http://example.com/link2"}}}
            }
        },
    ]
    result = get_all_links(records)
...
def test_transform_dict_nested():
    data = {
        "my~key": {"another/key": ["list_item", {"weird:key": "value"}]},
        "my/other": "valid",
    }
    result = transform_dict(data)
    # "my~key" => "my-key"
