...
from app.algos.operators.attribute import (
    AttributeParser,
    resolve_path_batch,
    EMBED_TYPES,
)
            ...
            [
                {
                    "commit": {
                        "record": {
                            "embed": {"images": [{"alt": "desc1"}, {"alt": "desc2"}]}
                        }
                    }
                }
            ],
            "embed.images[*].alt",
            np.array([["desc1", "desc2"]], dtype=object),
        ),
        (
            [{"commit": {"record": {"nested": {"key": "value"}}}}],
            "nested.key",
            np.array(["value"]),
        ),
        (
...
async def test_attribute_compare():
    ...
    result = await parser.attribute_compare(
        records, field_selector, operator, target_value
    )
...
async def test_register_operations():
    ...
    logic_evaluator.add_operation.assert_any_await(
        "attribute_compare", parser.attribute_compare
    )
