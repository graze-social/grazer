...
async def test_get_starter_pack():
    ...
    parser.network_workers[0].get_starter_pack.remote = AsyncMock(
        return_value="starter_pack_data"
    )
...
async def test_get_user_collection():
    parser = SocialParser()
    parser.network_workers = [MagicMock()]
    parser.network_workers[0].get_user_collection.remote = AsyncMock(
...
async def test_register_operations():
    ...
    logic_evaluator.add_operation.assert_any_await(
        "starter_pack_member", parser.starter_pack_member
    )
    logic_evaluator.add_operation.assert_any_await("list_member", parser.list_member)
    logic_evaluator.add_operation.assert_any_await(
        "magic_audience", parser.magic_audience
    )
