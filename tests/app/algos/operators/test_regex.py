...
@pytest.mark.parametrize(
    "records, field_selector, expected",
    [
        (
            [{"commit": {"record": {"text": "hello world"}}}],
            "text",
        ...
        (
            [{"commit": {"record": {"text": ["hello", "world"]}}}],
            "text",
            np.array(["['hello' 'world']"], dtype="<U17"),
        ),  # Match actual output
        (
            [{"commit": {"record": {"text": None}}}],
            "text",
            np.array(["None"], dtype="<U4"),
        ),  # Match actual output
        (
            [{"commit": {"record": {}}}],
            "text",
...
async def test_matches_operator():
    parser = RegexParser()
    parser.get_record_texts = MagicMock(
        return_value=np.array(["hello world", "goodbye"])
    )

    records = [
        {"commit": {"record": {"text": "hello world"}}},
        {"commit": {"record": {"text": "goodbye"}}},
    ]
...
async def test_allow_operator_word_boundary_respected():
    parser = RegexParser()
    parser.get_record_texts = MagicMock(
        return_value=np.array(["theapple", "an apple a day"])
    )
...
async def test_allow_operator_escaped_regex_characters():
    parser = RegexParser()
    parser.get_record_texts = MagicMock(
        return_value=np.array(["hello.world", "hello+world", "hello*world"])
    )

    result = await parser.allow_operator(
        None, "text", ["hello.world", "hello+world", "hello*world"]
    )
...
async def test_allow_operator_multiple_domains():
    ...
    parser.get_record_texts = MagicMock(
        return_value=np.array(
            ["Visit obsidian.md or notion.so for more.", "Nothing to see here."]
        )
    )
...
async def test_register_operations():
    ...
    logic_evaluator.add_operation.assert_any_await(
        "regex_matches", parser.matches_operator
    )
    logic_evaluator.add_operation.assert_any_await(
        "regex_negation_matches", parser.negation_matches_operator
    )
