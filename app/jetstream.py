...
class Jetstream:
        ...
        try:
            while True:
                try:
                    ...
                    logger.info(f"No more data for slice {start_us}->{end_us} (2s of silence). Moving on.")
                    break
                ...
                try:
                    ...
                    if msg_time > end_us:
                        logger.info(f"Got time_us={msg_time} > end_us={end_us}; closing in background.")
    ...
    @classmethod
    async def yield_jetstream_reversed(cls, end_cursor: int = None, start_cursor: int = None):
        """
        ...
        while current_end > start_cursor:
            ...
            async for record in Jetstream.fetch_minute_data(time_range_start, current_end):
                yield record
    ...
    async def graceful_close(cls, ws):
        ...
        try:
            await asyncio.wait_for(
                ws.close(code=1000, reason="End of slice"), 
                timeout=1
            )
        except asyncio.TimeoutError:
