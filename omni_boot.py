...
namespace = boot_settings.namespace
...
async def boot_cpu(num_cpus: float, num_gpus: float, num_workers: int):
    ...
    name = f"cpu:{namespace}"
    network_workers = discover_named_actors("network:", timeout=10)
    gpu_embedding_workers = discover_named_actors(f"gpu:{namespace}:embedders", timeout=10)
    gpu_classifier_workers = discover_named_actors(f"gpu:{namespace}:classifiers", timeout=10)
    cache = discover_named_actor("cache:", timeout=10)
